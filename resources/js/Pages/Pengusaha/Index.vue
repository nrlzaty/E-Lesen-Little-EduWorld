...
const OnDelete = (row) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            router.delete(route('pengusaha.destroy', { id: row })).then(() => {
                Swal.fire({
                    title: "Dipadamkan!",
...
watch(search, (newSearch) => {
    router.get(route('pengusaha.index'), { search: newSearch }, { preserveState: true });
});
const clearSearch = () => {
...
<template>
    <AppLayout title="Butiran Pengusaha">
        <template #header>
...
<section class="sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <a v-if="applicant && applicant.id" :href="route('pengusaha.create', applicant.id)">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4">Nama Pengusaha</th>
                            <th scope="col" class="px-4 py-3">No Kad Pengenalan</th> 
                            <th scope="col" class="px-4 py-3">Email</th>
                            <th scope="col" class="px-4 py-3">No Telefon</th>
                                <span class="sr-only">Actions</span>
                            
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(Pengusaha, index) in props.Pengusaha.data" :key="index" class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">{{ (props.Pengusaha.current_page - 1) * props.Pengusaha.per_page + index + 1 }}</td>
                            <th v-if="Pengusaha.applicant" scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ Pengusaha.applicant.nama }}</th>
                            
                            <td class="px-4 py-3">{{ Pengusaha.nama_pengusaha }}</td>
                            <td class="px-4 py-3">{{ Pengusaha.kad_pengenalan_pengusaha }}</td>
                            <td class="px-4 py-3">{{ Pengusaha.email_pengusaha }}</td>
                            <td class="px-4 py-3">{{ Pengusaha.telefon_pengusaha }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <div class="inline-flex rounded-md shadow-sm" role="group">
                                    <a :href="route('pengusaha.show', Pengusaha.id)">
        ...
        <div class="float-end">
             <Pagination class="mt-4" :links="Pengusaha.links"/>
        </div>
