...
const OnDelete = (row) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Only perform the delete if confirmed
            router.delete(route('setting.role.destroy', { id: row })).then(() => {
                Swal.fire({
                    title: "Dipadamkan!",
                ...
                Swal.fire({
                    title: "Ralat",
                    text: "Terdapat masalah semasa memadamkan peranan.",
                    icon: "error",
                });
            });
        } else {
...
router.get(route('setting.role.index'), { search: newSearch }, { preserveState: true });
});
const clearSearch = () => {
...
<section class="sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4">Nama Peranan</th>
                            <th scope="col" class="px-4 py-3">Kod Peranan</th>
                    ...
                    <tbody>
                        <tr v-for="(role, index) in props.Roles.data" :key="index" class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">{{ (props.Roles.current_page - 1) * props.Roles.per_page + index + 1 }}</td>
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ role.name }}</th>
                            <td class="px-4 py-3">{{ role.code_name }}</td>
                            <td class="px-4 py-3">{{ role.is_active ? 'Aktif' : 'Tidak Aktif' }}</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <div class="inline-flex rounded-md shadow-sm" role="group">
                                    <!-- Edit Button -->
                                    <a :href="route('setting.role.edit', role.id)">
        ...
        <div class="float-end">
             <Pagination class="mt-4" :links="Roles.links"/>
        </div>
    </div>
</section>
<!-- End block -->
