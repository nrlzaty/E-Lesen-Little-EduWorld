...
watch(search, (newSearch) => {
    router.get(route('status.perbaharui-lesen-telah-disemak'), { search: newSearch, status: route().params.status }, { preserveState: true });
});
...
const notiRenewTelahDisemak = ref([]);

onMounted(() => {
    fetchRenewTelahDisemakNotifications();
});

const fetchRenewTelahDisemakNotifications = async () => {
    try {
        const res = await axios.get('/dashboard-data');
        if (res.data && res.data.renewTelahDisemakNotifications) {
            notiRenewTelahDisemak.value = res.data.renewTelahDisemakNotifications;
        }
    } catch (e) {
...
<template>
    <AppLayout title="Senarai Perbaharui Lesen Telah Disemak">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
               Senarai Perbaharui Lesen Telah Disemak
            </h2>
        </template>

        <!-- Notification Block for Pegawai Perlulusan -->
        <div v-if="notiRenewTelahDisemak.length > 0" class="mb-6">
            <div class="bg-purple-100 border-l-4 border-purple-500 text-purple-700 p-4 rounded shadow">
                <div class="flex items-center mb-2">
                    <svg class="w-5 h-5 mr-2 text-purple-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" />
                    </svg>
                    <span class="font-semibold">Terdapat permohonan Perbaharui Lesen yang telah disemak oleh Pegawai Penyemakan untuk tindakan anda.</span>
                </div>
                <div class="ml-7">
                    <span>Klik nama permohonan untuk tindakan:</span>
                    <ul class="list-disc ml-5 mt-1">
                        <li v-for="item in notiRenewTelahDisemak" :key="item.id">
                            <a :href="route('status.show', item.id)" class="text-purple-700 font-semibold hover:underline">
                                {{ item.nama }}
                            </a>
                        </li>
                    </ul>
...
<section class="sm:p-5 antialiased">
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            <div class="overflow-x-auto">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
    <tr v-if="props.applicants.data.length === 0">
    ...
    <tr v-else v-for="(applicant, index) in props.applicants.data" :key="index" class="border-b dark:border-gray-700">
        ...
        <td class="px-4 py-3">
        <span v-if="applicant.expiry_date && applicant.expiry_date !== '0000-00-00 00:00:00'">
            {{
                (() => {
                    const dt = applicant.expiry_date;
                    if (!dt) return '-';
