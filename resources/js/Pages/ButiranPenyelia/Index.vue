...
const OnDelete = (row) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            router.delete(route('butiranpenyelia.destroy', { id: row })).then(() => {
                Swal.fire({
                    title: "Dipadamkan!",
...
watch(search, (newSearch) => {
    router.get(route('butiranpenyelia.index'), { search: newSearch }, { preserveState: true });
});
const clearSearch = () => {
...
<template>
    <AppLayout title="Butiran Penyelia">
        <template #header>
...
<section class="sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4">Nama Penyelia</th>
                    ...
                    <tbody>
                        <tr v-for="(butiranPenyelia,index) in props.butiranPenyelia.data" class="border-b dark:border-gray-700">
                            <th v-if="butiranPenyelia.applicant"scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ butiranPenyelia.applicant.nama  }}</th>
                            
                            <td class="px-4 py-3">{{ butiranPenyelia.nama_penyelia }}</td>
                            <td class="px-4 py-3">{{ butiranPenyelia.kad_pengenalan_penyelia }}</td>
                           
                            <td class="px-4 py-3 flex items-center justify-end">
                                
                                <td class="px-4 py-3 flex items-center justify-end">
                                <div class="inline-flex rounded-md shadow-sm" role="group">
                                    <a :href="route('butiranpenyelia.show', butiranPenyelia.id)">
        ...
        <div class="float-end">
             <Pagination class="mt-4" :links="butiranPenyelia.links"/>
        </div>
